...
void main() {
	...
	vec3 lightDir = (gubo.eyePos - fragPos) / length(gubo.eyePos - fragPos);
	vec3 lightColor = gubo.lightColor.rgb * pow((g / length(gubo.eyePos - fragPos)), beta) * clamp((dot(lightDir, gubo.lightDir) - cosout) / (cosin - cosout), 0.0, 1.0);
